document.addEventListener('DOMContentLoaded', function() {
    console.log('App Started: v73.6 (Iron-Clad Matrix & Full Functionality)');
    ...
    function updateDashboardStats() {
        const pfxs = ['math', 'eng', 'phys', 'chem', 'bio', 'it', 'eco', 'sat', 'ielts'];
        pfxs.forEach(p => {
    ...
    async function checkProfileAndTour() {
        const { data: user, error: userError } = await supabaseClient.from('users').select('*').eq('telegram_id', telegramUserId).maybeSingle();
        ...
        if (user) {
            ...
            const { data: newUser, error: createError } = await supabaseClient.from('users').insert({ telegram_id: telegramUserId, name: fullName }).select().single();
            if (newUser) { internalDbId = newUser.id; currentUserData = newUser; }
            else { console.error("Create User Error:", createError); return; }
        }
        ...
        if (!currentUserData || !currentUserData.full_name || !currentUserData.class) showScreen('reg-screen');
    ...
    async function handleStartClick() {
        ...
        if (!allQ || allQ.length === 0) { alert("Savollar topilmadi."); updateMainButton('start', currentTourTitle); return; }
    ...
    function updateMainButton(st, title = "") {
        ...
        else if (st === 'completed') { b.innerHTML = `<i class="fa-solid fa-check"></i> ` + t('tour_completed_btn'); b.onclick = () => showScreen('result-screen'); }
    ...
    async function loadLeaderboard() {
        ...
        scores.forEach((s, idx) => {
            ...
            listEl.insertAdjacentHTML('beforeend', `<div class="leader-card" style="${u.id === internalDbId ? 'border:1px solid var(--primary); background:#F0F8FF' : ''}"><div class="l-rank">${idx+1}</div><div class="l-info"><span class="l-name">${displayName}</span><span class="l-sub">${meta}</span></div><div class="l-score">${s.score}</div></div>`);
        });
    }
